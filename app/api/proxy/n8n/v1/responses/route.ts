...
// n8n's "Message a model" node with operation "response" calls /v1/responses.
// This route translates the Responses API format to chat completions and back.
...
function parseInput(input: ResponsesInput): Array<{ role: string; content: string }> {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { input, instructions, temperature, max_output_tokens } = body
    const productSlug = body.product_slug

    if (!input) {
      return Response.json(
        { error: { message: 'Missing required parameter: input', type: 'invalid_request_error', code: 'missing_parameter' } },
        { status: 400 }
      )
    }
    ...
    if (!userMessage) {
      return Response.json(
        { error: { message: 'No user message found in input', type: 'invalid_request_error', code: 'missing_user_message' } },
        { status: 400 }
      )
    }
    ...
    let systemPrompt = instructions || `Você é um assistente de suporte com acesso a ${contextName}.
...
Responda com base no seu conhecimento. Se não souber a resposta, diga claramente.`
    ...
    // Return in OpenAI Responses API format
    return Response.json({
      id: `resp_${Date.now()}`,
      object: 'response',
    ...
    console.error('n8n responses proxy error:', errMsg)
    return Response.json(
